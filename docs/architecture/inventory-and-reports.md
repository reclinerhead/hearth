...
- **`lib/reports/theme.ts`** — the shared print theme + document shell. `buildReportDocument({ title, address, bodyHtml, templateCss })` wraps a template's page-composition HTML in a full standalone document: the dark Hearth palette (mirrored from `globals.css` — the PDF renders in Hearth's theme, a product surface, not a white sheet), the three fonts pulled from Google Fonts via `@import` (Chromium renders outside Next's font pipeline), `@page` geometry (Letter, an oversized bottom margin to reserve the footer band), and the **running attribution footer**. `REPORT_COLORS` exports the palette for templates.
- **Running attribution footer (shared, every page).** A single muted JetBrains-Mono line — `Hearth — Home Awareness · Generated for {address} · Powered by ToddTech` — where "ToddTech" is a clickable PDF link to `TODDTECH_HEARTH_URL` (the Hearth portfolio page, not the site root). It's a `position: fixed` element, not Puppeteer's `footerTemplate`: fixed elements repeat on every printed page in Chromium's paged-media mode **and** keep their anchors as real clickable link annotations (the footerTemplate path renders in a separate context and produces no clickable links). The constants live in `lib/reports/constants.ts`.
...
3. **Involvement** — learn → act → involve, the free-testing affordance (only when `ccr_findings.free_testing_offer` is present), the utility contact block, and the once-at-the-end methodology + "awareness, not a substitute for testing" disclosure.

The route handler at `app/api/reports/water-quality/route.ts` loads the active house + the WQA finding (RLS-bound), requires CCR-derived data (422 otherwise — the card surfaces the message), computes the signature off `checked_at`, serves the cached PDF on a hit, else renders → persists best-effort → serves. The `/reports` card (`live: true` in the page's `GROUPS`) renders a client `WaterQualityGenerateButton` that fetches the route with a spinner and downloads the blob; every other card across all four groups stays disabled. The fourth `GROUPS` entry — **"Your home's habitat"** — is forward-compatible: the planned Superfund and FEMA reports join it.
