...
2. **Auth gate**: if there is no user and the route is not public (`/`, `/login`, `/auth/*`, `/api/cron/*`, `/how-it-works`, `/water` + `/water/*`, `/sitemap.xml`, `/robots.txt`), redirect to `/login`. `/api/cron/*` is public here because a Vercel Cron request carries no session cookie — without the exemption the gate would 307 it to `/login` and the handler would never run. Those handlers are not actually open: each gates on the `Authorization: Bearer ${CRON_SECRET}` header Vercel sends (see [storage-reconciliation.md](storage-reconciliation.md#scheduling-and-auth)). The `/water` and `/how-it-works` entries are the `app/(public)/` pages (see [public-pages.md](public-pages.md)) — matched segment-exact so a future `/water-adjacent` authenticated route isn't accidentally exempted; the sitemap/robots entries exist because crawler entry points must never bounce to `/login`.
