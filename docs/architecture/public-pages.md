...
5. **Utility contact: prefer the CCR's own testing line, fall back to the EPA admin, never an email from the admin record.** *(Amended alongside #303: the original "no admin contact at all" was relaxed so the civic "what you can do next" block can carry a callable contact.)* The block prefers the utility's **own free-testing contact as printed in its CCR** (the number a resident actually wants), gated through the validated-passthrough check in rule 4; failing that it shows the EPA-listed administrator **name and phone**. The admin **email is never published** (`water_systems.email_addr` is the harvest vector this rule most guards against, and adds the least over a phone). The contact carries no PWSID (rule 3).
...
**The utility contact block** (see hard rule 5) is a discriminated union: `ccr_free_testing` (the CCR's printed testing contact, when `sanitizeCcrContact` accepts it as a clean phone/email) is preferred over `epa_admin` (`{ name, phone }` from the EPA record). `sanitizeCcrContact` is the rule-4 carve-out — it accepts only a phone (7–15 digits, phone-shaped chars) or a simple email, rejecting URLs and free text so a crafted upload can't inject a link or string. The admin email is never carried either way.
