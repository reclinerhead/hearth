...
4. **No CCR extraction free-text on the page.** `extracted_data` is user-contributed content (whatever PDF someone uploaded); this is the content-injection defense — a crafted upload must not be able to place attacker-chosen text on an indexed, ToddTech-branded page. Public pages render full per-contaminant detail (issue #303 retired the old summary-level clause), but every rendered string is Hearth's own: names, descriptions, and EPA links resolve through the canonical contaminants reference (`findWqaContaminantByAlias`), units pass a small allowlist, and everything else is a validated number or one of our own labels. A displayed row that fails reference resolution or unit normalization is **omitted from the named list and rolled into an honest count** ("N more measured parameters appear in the report"). System names come from EPA's `pws_name`, never from the CCR. **One deliberate validated-passthrough exception:** the utility's CCR-printed free-testing contact may be surfaced in the next-steps block *only* when it validates as a clean phone number or email (`sanitizeCcrContact`) — a format that can't carry markup, a link, or a sentence; a `web` URL or any free-text value is rejected and the page falls back to the EPA admin contact.
5. **Utility contact = the EPA-published POC (name / phone / email) plus, when present, the CCR free-testing line.** *(Amended across #303: the original "no admin contact at all" was fully relaxed. The `water_systems` admin fields are public government records EPA publishes as the system's point of contact — not PII — so name, phone, AND email are all surfaced.)* The block shows the EPA POC and, alongside it, the utility's **own free-testing contact as printed in its CCR** when that value passes the rule-4 validated-passthrough check (clean phone/email only). The contact carries no PWSID (rule 3).
...
The privacy-sensitive subset is pinned by tests: `lib/public-pages/water-summary.test.ts` asserts the view model carries the EPA-published POC (name/phone/email) but never the PWSID, and that a CCR free-testing value only survives when it validates as a clean phone/email.
...
**The utility contact block** (see hard rule 5) carries two complementary sources, both shown when present: `admin` — the EPA-published POC `{ name, phone, email }` (public government records; gated on a phone or email being on file) — and `freeTesting` — the CCR's printed testing contact, but only when `sanitizeCcrContact` accepts it as a clean phone/email. `sanitizeCcrContact` is the rule-4 carve-out: it accepts only a phone (7–15 digits, phone-shaped chars) or a simple email, rejecting URLs and free text so a crafted upload can't inject a link or string onto the page. The EPA admin fields come from the structured `water_systems` record (not the uploaded PDF), so they carry no injection risk; only the CCR value needs the validator.
