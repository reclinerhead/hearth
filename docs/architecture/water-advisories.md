...
All in the `hearth` schema, migrations `20260922200000_create_water_advisories.sql`, `20260922230000_add_rss_source_kind_and_portage.sql`, and `20260924120000_kalamazoo_back_to_city_page_via_relay.sql` (data only: the Kalamazoo source row).
...
**Pacing through the relay.** The relay admits one fetch per upstream host every 3 s across all its tenants and gives Hearth 10 fetches a minute; a burst comes back `429` with `{"error":"host-throttled"}`. A detail URL is never fetched again once stored, so a throttled detail fetch would leave that advisory's `published_on` null for good, not just late. When a source fetches via the proxy, the OpenCities adapter therefore pauses `PROXY_DETAIL_PACE_MS` (3.2 s) before every detail fetch (the list fetch counts against the same host gate) and caps them at `PROXY_MAX_DETAIL_FETCHES` (8) per run, which keeps a seed run inside the route's 60 s budget. Direct fetches are unpaced with a cap of 12. Proxied fetches time out at 25 s instead of 20 s, so Hearth records the relay's own `504 upstream-timeout` (its origin deadline is 20 s) rather than racing it. A `429` is never retried (see the retry rule below).
