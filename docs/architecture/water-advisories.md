...
Vercel Cron (*/30)  →  /api/cron/water-advisories
   ...
   per enabled source:    ├─ adapter(kind, config)  → ParsedAdvisory[]     (I/O: lib/water-advisories/adapters/*)
                          ├─ carryForwardDetail()   → stored page fields   (pure: detail.ts — OpenCities only)
...
All in the `hearth` schema, migrations `20260922200000_create_water_advisories.sql`, `20260922230000_add_rss_source_kind_and_portage.sql`, `20260924120000_kalamazoo_back_to_city_page_via_relay.sql`, and `20260924210000_reseed_kalamazoo_for_detail_classification.sql` (the last two are data only: the Kalamazoo source row and its advisory rows).
...
|---|---|---|---|
| `rss` ([`adapters/rss.ts`](../../lib/water-advisories/adapters/rss.ts)) | `{ feed_url, keywords?, required_keywords?, exclude_keywords?, system_wide_phrases?, use_fetch_proxy? }` | Portage (`MI0005520`, its CivicPlus News Flash feed) | Throws on non-2xx, a non-XML body, a missing `rss/channel` or `feed` root, or **zero raw items**. A well-formed feed whose items all fail the keyword filter returns `[]` and is a **success** (a city with nothing current). |
| `opencities_list` ([`adapters/opencities-list.ts`](../../lib/water-advisories/adapters/opencities-list.ts)) | `{ list_url, system_wide_phrases?, use_fetch_proxy? }` | Kalamazoo (`MI0003520`, the city's Boil-Water-Advisories page, through the relay — see below) | Throws on non-2xx, on Akamai's "Access Denied" stub, and on a page that parses to **zero** list entries (the list has never been empty; zero means the markup changed or we got a challenge page). |

**RSS keyword filter.** `keywords` is any-of (default: boil water / do not drink / do not use / water advisory / water notice), matched against title + summary; `required_keywords` is all-of (for a regional feed such as a newsroom's, e.g. `["kalamazoo"]` when Kalamazoo ran on WMUK); `exclude_keywords` is none-of. Both RSS 2.0 and Atom parse (`fast-xml-parser`, no native deps); `published_on` is the item date as a UTC calendar date; the row is keyed by the item link. Matched keywords land in `raw.matched_keywords` for debugging.
...
**Kalamazoo runs on the city page, through the relay.** The city's own OpenCities page is the canonical record and carries street-level entries, but it sits behind Akamai and every cloud egress is blocked (see "Bot protection"). Its source row sets `use_fetch_proxy: true`, so every fetch goes through [toddtech-web-relay](https://github.com/reclinerhead/toddtech-web-relay), a self-hosted relay with residential egress. WMUK's open news feed (the local NPR station, one to three hours behind the city, no street-level entries) was the interim source until the relay existed; falling back to it is a source-row edit (`kind = 'rss'`, the WMUK `feed_url`, `required_keywords: ["kalamazoo"]`). Either direction, delete the source's advisory rows in the same migration so the switched source seeds silently.

**Pacing through the relay.** The relay admits one fetch per upstream host every 3 s across all its tenants and gives Hearth 10 fetches a minute; a burst comes back `429` with `{"error":"host-throttled"}`. An unseen URL's `published_on` is read once, on first sight, so a throttled first read would leave that date null for good, not just late. When a source fetches via the proxy, the OpenCities adapter therefore pauses `PROXY_DETAIL_PACE_MS` (3.2 s) before every detail fetch (the list fetch counts against the same host gate) and caps them at `PROXY_MAX_DETAIL_FETCHES` (8) per run, which keeps a seed run inside the route's 60 s budget. Direct fetches are unpaced with a cap of 12. Under the cap, unseen URLs are read first, then the open advisories being re-read (below); an open row past the cap simply keeps its stored detail for that run. Proxied fetches time out at 25 s instead of 20 s, so Hearth records the relay's own `504 upstream-timeout` (its origin deadline is 20 s) rather than racing it. A `429` is never retried (see the retry rule below).
...
**The advisory's own page (issue #355).** The list entry's title and blurb are a separate field the city writes once and does not maintain: Kalamazoo lifted the September 2026 district-wide advisory by editing the advisory's page in place (retitled "…LIFTED…", "This advisory has been lifted." prepended) while the list still read the first-day text, and it posts dated updates the same way. So the detail page, not the list entry, is where a change shows. The adapter reads it (best-effort, capped and paced as above) for **unseen URLs** and for every **open** stored row — status active / scheduled / unknown, dated within the timeline's 14-day open window (`selectRefreshUrls` in [`detail.ts`](../../lib/water-advisories/detail.ts), sharing `DEFAULT_OPEN_WINDOW_DAYS` with `timeline.ts`). A lifted row is final and a stale one is history; neither costs a fetch. From the page it takes `Published on Month D, YYYY` (first sight only), the `h1`, and the **lead** — the first two text-bearing `h2`/`h3`/`p` blocks after the title, skipping the published-on line, capped at 400 chars (`parseDetailLead`) — and hands them on as the typed `detail` field, persisting them in `raw.detail_title` / `raw.detail_lead`. Status and the content hash read `detail` (see below). **Carry-forward:** an entry whose page was not read this run — known and not open, past the cap, or a failed fetch (`raw.detail_error`) — takes the stored capture's detail fields before classification (`carryForwardDetail`), so detail changes only on a successful fresh read and a relay `504` on one run can never flip a status or fire an "updated". Rows from sources with no detail page (RSS) carry no `detail` at all.

**Bot protection (load-bearing).** The Kalamazoo city site sits behind Akamai Bot Manager, which scores on source IP reputation. Tested 2026-09-22: a home connection running Node `fetch` → 200; Vercel production (AWS) → 403; GitHub Actions (Azure) → 403; a Cloudflare Worker → 403. Assume every cloud egress is blocked. The escape hatch is a fetch proxy: `WATER_ADVISORY_FETCH_PROXY_URL` is a template with a `{url}` placeholder, today pointing at [toddtech-web-relay](https://github.com/reclinerhead/toddtech-web-relay) (`https://<relay-host>/fetch?key=KEY&url={url}`; the host and Hearth's tenant key live only in Vercel, never in this repo), and a source opts in with `use_fetch_proxy: true` in its config — only the sources that need it pay the hop; open feeds fetch directly. Through the proxy the adapter drops its browser-like headers (the proxy supplies its own fingerprint). The relay follows redirects itself and returns the origin's status and body unwrapped, so detection is the same either way: a denial is detected by status *or* by the "Access Denied" / reference-id markers in the body, and the thrown error carries the upstream server (the `server` header, else the relay's `x-relay-upstream-server`, since the relay forwards only `content-type`/`etag`/`last-modified` from the origin) and a 160-char tag-stripped excerpt so the admin page and the alarm email say *why* without anyone re-running the fetch. The relay's own contract (headers, blocked codes, the per-tenant bucket and per-host gate) is in its `docs/TechnicalGuide.md` §§ 3 and 5. The health alarm below is the backstop for the day whichever path is in use stops working.
...
- **Status**: when the advisory's own page has been read (`detail`), `lifted` if its heading says lifted/rescinded or its lead says "has been / is / was lifted" — checked first, because the list entry keeps its first-day wording after an in-place lift. Otherwise from the list title (then summary): `lifted` by the same two rules, else `scheduled`, else `active` for advisory/order/notice/do-not-drink wording, else `unknown`. Only the lifted check reads the page; a page with a neutral lead never overrides a scheduled or active list title.
...
- **Events.** `issued` — a new URL with status active/scheduled/unknown. `lifted` — a new URL with status lifted, or an existing row whose status flips to lifted (takes precedence over updated). `updated` — an existing URL whose `content_hash` changed. An entry that disappears from the page is left alone; it never notifies.
- **Content hash** = SHA-256 over the normalized list title + summary, plus the detail heading + lead **only when the entry carries `detail`**. Status is not part of it. Without `detail` the digest is byte-identical to the original formula (pinned by test), so RSS rows never re-hash on a deploy; with it, an edit to the page's lead — the city's dated updates — is an `updated`.
...
- **`raw` merges.** The stored capture is merged under the fresh parse on existing rows, so detail-page fields (`detail_title`, `detail_lead`) survive a run that did not re-read the page; the fresh parse wins for the keys it carries.
...
Resend via the `resend` package; `RESEND_API_KEY` + `WATER_ADVISORY_FROM_EMAIL` (a verified ToddTech sender). Plain text + minimal inline-styled HTML, no template library. Four messages: **issued**, **updated**, **lifted** (the city's title and a quoted, attributed passage, the published date, a link to the city's page, boil/bottled guidance on issued/updated, the "{admin} added you" line, a one-click unsubscribe link), the **confirmation** (double opt-in), and the **watcher alarm** / recovery note. The quoted passage is the advisory page's lead (`raw.detail_lead`) when the row has one, else the list summary: after an in-place lift the list blurb still says "should not drink the water without boiling", which must not sit under a "lifted" subject. Every scraped string is HTML-escaped before it reaches a template.
...
| `WATER_ADVISORY_FETCH_PROXY_URL` | unset (direct) | Proxy template with `{url}`; used only by sources with `use_fetch_proxy: true`. Set in production to the relay's `/fetch?key=…&url={url}` template; Kalamazoo depends on it. |
