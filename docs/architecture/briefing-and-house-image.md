...
- **Four milestones, in fixed order:** add a home photo (`YOUR HOME`), record an emergency video (`EMERGENCIES`), add a first appliance (`INVENTORY`), review habitat findings (`HABITAT`). Each card is a **full-bleed image tile** modeled on the Emergency panel's `PrimaryTile` (issue #218) — a portrait `3/4` card with full-bleed category art, a prominent ~56px top-left glyph chip over the art, a bottom gradient scrim, white overlay text (area eyebrow, awareness-framed lead title, secondary line), and a solid amber CTA pill with a trailing arrow as the brightest element. The grid runs `md:grid-cols-4` → `sm:grid-cols-2` → `grid-cols-1`; the whole tile is the click target (matching `PrimaryTile` / `InventoryTile`), with the pill as the visible affordance. Each milestone carries an optional `imageSrc` on the content map, pointing at painterly category art under `public/onboarding/` (`house.jpg`, `emergency.jpg`, `inventory.jpg`, `habitat.jpg`); when absent the art layer is skipped and the glyph sits over a flat warm `--color-bg-surface-raised` field, so a missing asset never shows a broken image. While any milestone is still pending the grid shows **all four** tiles in fixed order; a `complete` milestone persists as a green "done" tile alongside the pending ones (so the user sees their progress and the grid stays full) until every milestone is done, at which point the whole panel hands off to the retire beat. The completed tile's art desaturates and dims, its glyph chip goes green, and the amber CTA is replaced by a non-interactive muted-green check pill (`Added` / `Recorded` / `Reviewed`).
...
`page.tsx` computes the four booleans server-side — two cheap `head:true` counts run in parallel; the photo and habitat signals read off the house row already fetched — and hands them to `buildMilestones(signals)`, which resolves each card's `pending`/`complete` state. The panel renders all four while any remain pending — pending tiles carry their amber CTA, complete tiles render the green "done" treatment — and retires only once every milestone is complete. Completion is one-way: a completed milestone's signal is never re-evaluated to re-open it even if the underlying data later changes (the learning moment already happened); there is no per-card dismiss.

### Why only habitat needs stored state

*Viewing* habitat findings is not a write, so it has no natural data signal. That one milestone reads `habitat_reviewed` off the **`hearth.houses.onboarding_state jsonb`** column (migration `20260530180000_add_onboarding_state_to_houses.sql`, `not null default '{}'`). The blob is reserved strictly for view-event milestones and one-time UI beats — write-event milestones are deliberately *not* stored there, because deriving them is self-healing while a JSONB stamp every add-path has to remember is a drift risk. The shape is open-ended (`{ habitat_reviewed?: boolean, foundation_celebration_seen?: boolean }` today) so it extends without another migration — the `foundation_celebration_seen` key (issue #269) rode in on this open-endedness with no schema change.
...
**Celebration mechanics (durable, once-ever — issue #269).** Gated by the **durable** `foundation_celebration_seen` flag on `houses.onboarding_state`, decided server-side and passed to the panel as `celebrationSeen`. The celebration auto-shows exactly once — on the final-flip load when `celebrationSeen` is still false — and never re-surfaces in a later session. (It was previously gated by a per-tab `sessionStorage` flag, `hearthMilestonesRetireBeat:<houseId>`, which re-revealed the beat every new session — the regression #269 fixes.) Because the flag is server state there is no hydration dance: SSR and the first client render agree. On the reveal load, an effect calls the fire-and-forget, idempotent [`markFoundationCelebrationSeenAction`](../../app/actions/houses/mark-foundation-celebration-seen.ts) (mirrors `mark-habitat-reviewed.ts`) to stamp the flag so the next load hides it. A **dismiss `x`** (top-right, in both modes) hides the celebration immediately for this load via local `dismissed` state — the stamp is what keeps it gone afterward; in reopen mode it just closes the reopened panel.
