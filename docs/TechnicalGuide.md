...
**`renderOverviewBody` slot signature was extended** to pass a `{ houseId }` context: `renderOverviewBody?: (row, ctx: { houseId }) => ReactNode`. The Wqa overview body needs `houseId` to scope the CCR upload modal; passing it via a context object keeps room for future additions without churning the signature again. `HabitatFindingModal` and `HabitatFindingTrigger` both accept `houseId` and thread it through.
