...
app/                       # Next.js App Router
  ...
  (public)/                # Unauthenticated public group (epic #298)
    layout.tsx             # Lightweight public frame (wordmark, ToddTech footer)
    how-it-works/          # Living methodology page (moved from (app))
    water/[systemSlug]/    # Place-keyed public water quality pages (SSG + ISR)
  sitemap.ts               # Public-surface sitemap (slug registry driven)
  robots.ts                # Allow public surfaces, disallow session routes
...
- **Form controls never compute below 16px on touch.** `--text-body` is `clamp(14px, 1.5vw, 16px)`, which floors at **14px** on any phone-width viewport, and iOS Safari zooms the viewport whenever a focused `input` / `select` / `textarea` computes under 16px. `app/globals.css` carries the floor as `@media (pointer: coarse), (max-width: 639.98px)`; the condition is a union so the rule covers iPad portrait (768px wide but still 14px body text) while staying exercisable in a desktop browser at narrow widths. Two consequences worth knowing: an inline `style={{ fontSize }}` outranks the floor, so **form controls must not hardcode a font-size** — let them inherit; and the fix for auto-zoom is always the floor, never `maximum-scale=1` / `user-scalable=no` on the viewport meta, which would buy the same result at the cost of pinch-zoom for low-vision users. Issue #311; the zoom was first observed trapping the Smart Uploader's fixed-height page-sheet wider than the visual viewport.
