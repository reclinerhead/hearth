...
Schema foundation landed in issue #122. The **synthesis pipeline** plus the **Build maintenance plan** button on the inventory detail page landed in issue #126. The **direct-event pipeline** that auto-creates renewal tasks from documents carrying `metadata.expiration_date` landed in issue #131. The **"On your plate" panel** (dashboard + inventory detail page) and the **renamed History section** on the inventory detail page landed in issue #133. Still to ship against this schema: the task detail modal, the mark-renewed sheet, and the "renewed via document upload" toast.
...
- **Item empty state, `kind: "needs_research"`** — the inventory item has no `ai_insights.maintenance`. Renders the "Run Research on this item first" message; no primary CTA (the page's Research panel above already has that).
- **Item empty state, `kind: "no_plan_yet"`** — `ai_insights.maintenance` is populated but no synthesis run yet. Renders the "build a plan" message pointing at the existing Build button in the title row, rather than duplicating the action inline (the title-row button is the single source of truth for the action).

The page.tsx server component computes the discriminator (`hasActionableInsights`) from the loaded `ai_insights` and hands it to the inventory wrapper, which in turn passes the right `ItemEmptyState` discriminator to the panel. The panel chooses the empty branch over the populated branch when the tasks array is empty.
