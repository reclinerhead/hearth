...
**Branch logic, module integration, Smart Uploader path, action-layer wiring, summarization helpers, and severity changes from CCR data — all still in flight.** The migrations, schema, prompt, and prompt-version constant are the data plumbing; the consumer side (the `cws_with_ccr` branch becoming reachable, the CCR cache lookup running before SDWIS fetches, the post-upload contributor acknowledgment, the activity-log narration for CCR cache hit/miss) lands as the remaining commits on the same WQA-3 PR.
