...
**`hearth.documents.kind` CHECK constraint extended** in the same migration to include `'water_quality_report'`. The Smart Uploader will route CCR uploads via the new path picker entry; the document row carries the discriminator that dispatches to `analyze-ccr` rather than the receipt-analysis path.
...
**Still pending for WQA-3 completion:** the Smart Uploader path-picker entry, the `use-ccr-upload` hook, the `ccr-upload-flow` and contributor-acknowledgment UI, and the real-world smoke test against Kalamazoo's 2024 CCR. The findings view's "Detected in your water" section that renders `ccr_findings` is a follow-on (likely WQA-4-style overview-body changes).
