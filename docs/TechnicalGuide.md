...
The schema supports many houses per owner; the foundation issue (#98) wired the app layer up to match while shipping no user-visible change. Three pieces:
...
- **Capability gate** — [`lib/houses/capabilities.ts`](../lib/houses/capabilities.ts) exports `resolveUserCapabilities(supabase)`, called once per request from the (app) layout. Returns `{ canCreateAdditionalHouse, canSwitchHouses, isAdmin, planTier }`. `canCreateAdditionalHouse = isAdmin || planTier === "premium"` is the single chokepoint future surfaces (the "+ Add property" entry in the switcher, the `/houses/new` server action) consult before allowing a new house. `canSwitchHouses` is purely "does the user have ≥ 2 houses" so the switcher can render as a chip vs. a dropdown. When trial / referral / contractor-gets-N-free policies land, `resolveUserCapabilities` is the only file that changes.
...
**What this issue did NOT ship.** No house-switcher UI in the top nav, no `/houses/new` route, no proxy gate split for the "add property" flow, no plan-tier billing surface. The proxy's onboarding gate still redirects to `/onboarding` whenever `houses.count === 0`; the follow-up issues introduce `/houses/new` and refine the redirect. The capability object is plumbed through `AppShell → TopNav` as an unused-for-now prop so the follow-up UI is a one-file change.
...
- **Multi-house** UI. The foundation is in place (profile columns, `resolveActiveHouseId`, `resolveUserCapabilities`, capability plumbing through `AppShell` / `TopNav` — see "Multi-house" above), but the user-visible surfaces are not: no house-switcher dropdown, no `/houses/new` route, and the proxy's onboarding gate still locks new users to "one house before you can use the app".
