...
- **Hero photo** comes from the most-recently-attached document for the inventory item (same query shape as the dashboard's `InventoryPreview`). The server component hands the optimized 1920px storage path (with the 600px thumb as fallback) to `InventoryDetailView`, which signs it client-side via the shared `useCachedSignedUrl` hook so the URL string is cached in `sessionStorage` across navigations — same pattern the house image uses (see "Signed URL caching" below). A small neutral skeleton occupies the slot while the URL resolves on cache miss, so a known-present photo never briefly flashes the "Add photo" placeholder.
