...
The first end-to-end data flow in the product: a signed-in user with no house lands on `/onboarding`, captures their address via Mapbox, answers two property-situation questions on `/onboarding/property-details`, and is redirected to `/dashboard`.
...
app/(app)/onboarding/
  ...
  property-details/
    page.tsx                            # Server; resolves the active house, renders the form
    property-details-form.tsx           # Client; water source + basement segmented controls
```
...
- **`actions.ts`** (`"use server"`) — `createHouseFromMapboxFeature(feature)`. Verifies the session, calls `extractAddress(feature)`, inserts into `hearth.houses` with `owner_id = user.id` and `mapbox_raw = feature`. On Postgres unique-violation (code `23505`) the user already has this house — redirect to `/dashboard` rather than surface a SQL error. On success: writes `active_house_id` on `public.profiles` to point at the newly inserted house, kicks off the Day One Briefing workflow, then `redirect("/onboarding/property-details")` (the property-situation capture step, see "Property-details step" below). The `houses_seed_default_rooms` trigger seeds the 9 default rooms — no app code needed.
...
### Property-details step (issue #142)

The address form's server action redirects to `/onboarding/property-details` instead of `/dashboard` after the house insert. That page asks two property-situation questions — water source (private well / city or utility / shared system / not sure) and whether the home has a basement (yes / no / not sure). Downstream environmental modules read these off `HouseContext`: the Superfund label-calibration and recommended-actions follow-ups (#144, #149) gate "test your well" vs. "review your utility's CCR" off `water_source`, and the vapor-intrusion action only fires when `basement_present === true` and a nearby site carries VOCs.

- **`property-details/page.tsx`** (server) — Reads the current user, calls `resolveActiveHouseId(supabase)` to find the house the user just created (or their already-active house, on direct navigation). Fetches `id, address_line1, city, state, water_source, basement_present` and renders the client form pre-populated. Bounces to `/onboarding` when no active house resolves and to `/dashboard` when the SELECT comes back empty (race with delete, RLS edge case).
- **`property-details-form.tsx`** (client) — `<PropertySituationFields>` from [`components/property-situation-fields.tsx`](../components/property-situation-fields.tsx) renders both segmented controls. Save runs an RLS-bound UPDATE through the browser Supabase client (same pattern as `EditHomeDetailsModal`) and `router.push("/dashboard")`. "Skip for now" is a plain `<Link href="/dashboard">` — there's nothing to persist when the user opts out.

The two questions are framed as optional because they're slightly intimate; forcing an answer would push users to guess, and a confidently-wrong value is worse than `null`. Users who skip can fill them in later via the home-details edit modal, which renders the same `<PropertySituationFields>` block in its "Property situation" section (so existing users — including users who onboarded before this step landed — can populate the fields without re-onboarding and losing their inventory).

The `/houses/new` flow runs through the same `createHouseFromMapboxFeature` action, so adding a second property also funnels through `/onboarding/property-details`. The page is house-agnostic: it always edits the active house, which is the freshly inserted one by the time the redirect fires.
...
**Why no file-based debug log (yet).** The inventory-insights, serial-decode, and maintenance-synthesis pipelines each land a `logs/*-prompts.log` developer file via `node:fs/promises` and `node:path`. The Superfund summary call deliberately does *not* — the workflow bundler statically traces every file reachable from `workflows/habitat.ts → registry → epa-superfund-proximity/*` and blocks Node.js built-ins outside `"use step"` boundaries. The maintenance-synthesis log works because its dynamic import sits inside a `"use step"` function in the workflow file itself; the Superfund AI call sits inside the module's `check()`, which is *invoked* from a step (`runOneModule`) but isn't a step boundary in source. A follow-up that exposes a generic post-check enrichment slot on `HabitatModule` (so the AI call can live in a habitat-workflow step rather than inside the module) is the natural home for a file-based debug log.
...
**The reasoning expand — brand-thesis-made-visible.** [`<WhyThisTaskExpand>`](../components/maintenance/why-this-task-expand.tsx) is the modal's headline surface. Defaults closed (the modal opens calm — the user sees title and due-date first, depth is one tap away). The collapsed label switches between "Why this task" and "Why this practice" via a `variant` prop so per-use rows read naturally. When the task has modifiers, a small accent-chip count ("2 adjustments") appears on the collapsed label so the user can tell a habitat-adjusted task apart from a vanilla one at a glance — the discovery affordance for the depth that's inside. The open state renders `source_kind` in plain words, the `cadence_basis` prose, the cadence in plain language (Monthly / Every 6 months / Annually / Every 2 years), each modifier as a tinted row with a "See finding" link when `finding_module_key` is populated (the link uses the `/dashboard#habitat` hash anchor — habitat tiles don't have individual deep-link routes yet, the dashboard anchor is good enough for v1), and the anchor detail.
