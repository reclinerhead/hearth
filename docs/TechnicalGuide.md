...
**`Detected in your water` section now reads from `ccr_findings`** on the `cws_with_ccr` branch. The summarizer already sorted the contaminants concern → caution → context, so the renderer maps straight off the array. Lead/copper are folded into the same list (they're regular CCR rows), so the SDWIS LCR-only rendering is suppressed on this branch. Other CWS branches (`cws_no_ccr`, `non_community`) fall back to the SDWIS lead/copper view. Each rendered row branches on `has_multiple_observations` (issue #200): the common single-observation case renders as a single card; an analyte reported in more than one CCR table renders as one card with a "Measured under more than one program" disclosure that surfaces the alternate observation(s) and their monitoring context.
