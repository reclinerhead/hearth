...
**`Detected in your water` section now reads from `ccr_findings`** on the `cws_with_ccr` branch. The summarizer sorts contaminants concern → caution → context; within a tier (issue #199), rows are ordered by `detected_level / mcl` descending so the worst exceedance floats up, with rows whose ratio is uncomputable (no MCL on the row, or null detected level) sinking below ratio'd rows in extraction order. Lead/copper are folded into the same list (they're regular CCR rows), so the SDWIS LCR-only rendering is suppressed on this branch. Other CWS branches (`cws_no_ccr`, `non_community`) fall back to the SDWIS lead/copper view. Each rendered row branches on `has_multiple_observations` (issue #200): the common single-observation case renders as a single card; an analyte reported in more than one CCR table renders as one card with a "Measured under more than one program" disclosure that surfaces the alternate observation(s) and their monitoring context. The renderer also collapses the context tier behind an "N more contaminants detected at low levels" disclosure when there are 2+ context rows (issue #199 item 2) so concern/caution stay visible above the fold; a single context row stays inline. Tier badges are wrapped in the `Tooltip` primitive (issue #199 item 4) so hovering "Worth knowing" or "Context" surfaces the underlying threshold logic in place — the labels alone aren't legible to a non-expert. Health-risk weighting beyond the MCL ratio (MCLG=0 surfacing for "no known safe level" contaminants) is deliberately deferred to a future pass — see issue #199 item 3.
