...
The date picker wraps [`react-day-picker`](https://react-day-picker.dev) v10 in single-selection mode with `captionLayout="dropdown"` so the month and year captions become small chip-styled dropdowns alongside the prev/next chevrons. The dropdowns are bounded by `startMonth` / `endMonth` (100 years back / 10 years forward from today) so the user can jump to a far-back year in two clicks instead of click-stepping the chevrons month-by-month — older homes' purchase dates and the rare pre-2000 appliance install date are both reachable without finger-cramp. The library's own stylesheet is **not imported** — the `.rdp-*` classes the library emits are styled from scratch in [app/globals.css](../app/globals.css) so the day grid, navigation chevrons, the caption dropdown chips, weekday headers, today indicator, and selected-day fill all match Hearth's palette. The selected day uses `var(--color-accent)` as a solid fill; today (when not selected) gets an inset accent-tinted ring; out-of-month cells are dimmed.
...
The month picker is custom (no library). The popup is a year header — `◀ 2026 ▼ ▶` — above a 4×3 grid of month cells (`Jan` through `Dec`). The center element is a `<select>`-backed dropdown chip styled to match the date picker's caption dropdowns; the flanking chevrons handle ±1 year nav with `disabled` states at the bounds. Same 100-back / 10-forward year range as the date picker. Selected month cells fill with the accent; today's month-year combination gets the same inset accent ring as today's day in the date picker; hover/focus on any cell brightens it with the accent-tinted background mix the rest of the surface-ai surfaces use.
...
- **Property delete flow.** No UI to remove a property today; `resolveActiveHouseId`'s fallback path silently moves the user to another owned house if `active_house_id` ever dangles, but there is no way to retire a property. Needs careful design around what happens to attached documents and inventory.
