...
The WQA module's onboarding line is a richer case: it has *two* independent axes (EPA compliance violations and Lead and Copper Rule sample results) and the severity is the worse of the two. A pre-#186 implementation read only the compliance axis, which produced a contradictory row in the discovery modal whenever LCR alone pushed severity to `caution` ("no active compliance issues." next to a warning glyph and a "Worth knowing" pill). Issue #186 replaced that with a pure builder at [`lib/habitat/modules/water-quality-awareness/onboarding-message.ts`](../lib/habitat/modules/water-quality-awareness/onboarding-message.ts) that reads `finding.severity` + `system_card.compliance_status_short` + `system_card.has_active_non_health_based` + `classifyLcrAxis(lead_copper_summary)` and produces a two-clause sentence — clean axis as reassurance, flagged axis as the honest call-out. Lead vs. copper is distinguished when a single metal is responsible; "lead and copper" appears only when both axes are at the same tier. The builder is unit-tested per-row in `onboarding-message.test.ts`. The non-CWS branches (private well / cws_unmapped / stale / non_community) stay shaped by their own copy and don't go through this builder.

**Why `has_active_non_health_based` is persisted separately.** `compliance_status_short` is health-based-only by design — see `compliance.ts:158-159`, which sets it to `active_violations` only when `activeHealth > 0`. The framing is intentional: the homeowner-facing "is there an active compliance issue?" surface doesn't escalate on monitoring/reporting lapses. But `deriveSeverity` still flags `caution` for a non-health-based active violation, so without a separate field on the persisted system_card the discovery-modal builder would have no signal to distinguish "clean utility" from "clean except for paperwork" — and the row would silently fall through to the neutral fallback even though severity was caution. Issue #186 added the field, populated from `enrichment.compliance.has_active_non_health_based` at persist time, treated as `false` when absent (back-compat for rows persisted before the field shipped — they degrade to the neutral fallback until the WQA yearly cadence or a manual refresh re-runs the check).
