...
- **`actions.ts`** (`"use server"`) — `createHouseFromMapboxFeature(feature)`. Verifies the session, calls `extractAddress(feature)`, inserts into `hearth.houses` with `owner_id = user.id` and `mapbox_raw = feature`. On Postgres unique-violation (code `23505`) the user already has this house — redirect to `/dashboard` rather than surface a SQL error. On success: writes `active_house_id` on `public.profiles` to point at the newly inserted house (so both onboarding-first-house and add-property second-house land on the right dashboard), kicks off the Day One Briefing workflow, then `redirect("/dashboard")`. The `houses_seed_default_rooms` trigger seeds the 9 default rooms — no app code needed.
...
Schema foundation landed in issue #122. The **synthesis pipeline** plus the **Build maintenance plan** button on the inventory detail page landed in issue #126. The **direct-event pipeline** that auto-creates renewal tasks from documents carrying `metadata.expiration_date` landed in issue #131. The **"On your plate" panel** (dashboard + inventory detail page) and the **renamed History section** on the inventory detail page landed in issue #133. The **per-use cadence kind** and the **synthesis consolidation pass** landed in issue #135. Still to ship against this schema: the task detail modal, the mark-renewed sheet, and the "renewed via document upload" toast.
...
The dashboard's right column and the inventory detail page both host the same `<MaintenancePanel>` client component. The dashboard surface is house-scoped and **capped at the next 30 days** (the panel is a "what's on plate now" reminder, not a complete log); the inventory-detail surface is item-scoped and uncapped (the user is already inside a single item's context). Each surface has its own server-rendered data wrapper — [`app/(app)/dashboard/maintenance-panel-dashboard.tsx`](../app/(app)/dashboard/maintenance-panel-dashboard.tsx) and [`app/(app)/inventory/[id]/maintenance-panel-item.tsx`](../app/(app)/inventory/[id]/maintenance-panel-item.tsx) — that runs the appropriate Supabase queries and hands the result to the shared panel. Splitting fetch from render means the dashboard wrapper can take the `house_id` route and use the partial index `maintenance_tasks_house_open_by_due_idx` while the inventory wrapper takes `inventory_id` and uses `maintenance_tasks_inventory_open_by_due_idx`, without the rendering component needing to know which query ran. The dashboard query also joins through `inventory_id` to pull the item's `name` so each row can be labelled "DISHWASHER · Check and refill rinse aid" — at a glance the user shouldn't have to guess which appliance a task belongs to. The item wrapper sets `inventoryName: null` per row (the item context is already established by the page).

Both wrappers carry a `.neq("cadence_kind", "per_use")` filter so per-use practices stay off the date-anchored panel and surface only in the inventory-detail page's dedicated "Every time you use it" section (issue #135). PostgREST's `.neq` is a strict not-equals that excludes nulls — fine for our data because every synthesis-emitted row has a `cadence_kind`. If a future writer ever inserts rows with a null cadence_kind we'd revisit (the filter would silently drop them today). The completed-this-year count and the most-recent-completion footer don't carry the filter: per-use tasks are standing practices that never reach `status='completed'` in the lifecycle sense, so they can't slip into those counts.

**Dashboard cap + the `/maintenance` destination.** The dashboard query filters with `next_due_at <= today + 30 days`, which keeps the panel a quick-glance surface even as the synthesis pipeline produces dozens of long-cadence tasks per house. The trailing area of the section header carries a **View all** link in the house-scope branch (the `N total` chip is suppressed there — it would misrepresent the broader open list now that the panel is filtered). The link points at [`app/(app)/maintenance/page.tsx`](../app/(app)/maintenance/page.tsx), which is a TBD placeholder today. The route is real so the link works; the full content surface (every open task with time-window and per-item filters) lands in a later issue. The item-scope branch keeps the `N total` chip — that count is honest because the item-scope query is uncapped.
...
### "Every time you use it" section on the inventory detail page (issue #135)

The inventory detail page's per-use surface is [`app/(app)/inventory/[id]/per-use-practices.tsx`](../app/(app)/inventory/[id]/per-use-practices.tsx) — a server component that selects open `cadence_kind='per_use'` rows for the current item and renders them as static, non-interactive rows. The component returns `null` when the item has no per-use rows, so items without per-use practices keep their existing layout cleanly. The page server component passes it in as a `perUsePracticesSlot` ReactNode to the client `InventoryDetailView`, same pattern as `maintenancePanelSlot`. Section order on the detail page: hero + tiles → research → on-your-plate panel → **every time you use it** → documents/notes grid → history.

Rows are not buttons in this phase (no detail modal yet — that lands in the phase 6 task detail modal work). The icon is `refresh-cw` for v1, matching the consumable-flavored kinds that dominate today's per-use practices (rinse aid, lint screen). When per-use inspections become more common (water heater drip check, sump pump operation test) the icon-per-kind logic can mirror what `MaintenanceTaskRow` does — for now one icon is honest about the v1 vocabulary.
...
The disabled "Log maintenance" placeholder button is gone — task creation flows through synthesis or the direct-event pipeline now, and per-task completion is the future mark-renewed sheet's job (issue #6). When the item has no completed tasks and no install/purchase dates yet, the section falls back to a single "No history yet" timeline row rather than rendering an empty `<ol>`.
