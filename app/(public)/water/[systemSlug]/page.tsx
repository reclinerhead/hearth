...
import { formatAdvisoryDate } from "@/lib/water-advisories/timeline";
...
export default async function PublicWaterSystemPage({
  ...
  return (
    <article className="flex flex-col" style={{ gap: "var(--space-7)" }}>
      ...
      <AdvisoriesSection summary={summary} entry={entry} />
...
/**
 * Recent boil-water advisories from Hearth's watcher (issue #347). Only
 * for systems Hearth watches; oldest first so the newest sits at the
 * bottom, with an open advisory featured in the warning tone. Server-
 * rendered, no client JS — the page stays fully static and is
 * regenerated by the watcher only when it records a change.
 */
function AdvisoriesSection({
  ...
  const { advisories } = summary;
  if (advisories.kind === "not_watched") return null;

  return (
    <section>
      <SectionHeader eyebrow="Boil water advisories" title="Recent advisories" />
      {advisories.kind === "unavailable" ? (
        <div className="surface" style={{ padding: "var(--space-5)" }}>
          <p style={{ margin: 0, color: "var(--color-text-secondary)" }}>
            Hearth watches {entry.shortPlace}&rsquo;s advisories, but the record
            wasn&rsquo;t reachable when this page was last generated. It refreshes
            automatically.
          </p>
        </div>
      ) : advisories.items.length === 0 ? (
        <div className="surface" style={{ padding: "var(--space-5)" }}>
          <p style={{ margin: 0, color: "var(--color-text-secondary)" }}>
            No boil water advisories recorded since Hearth began watching on{" "}
            {formatAdvisoryDate(advisories.watchingSince)}.
          </p>
          <AdvisoriesSourceNote note={advisories.sourceNote} />
        </div>
      ) : (
        <div className="surface" style={{ padding: "var(--space-5)" }}>
          <ol
          ...
          >
            {advisories.items.map((item) => (
              <PublicAdvisoryRow key={item.key} item={item} />
            ))}
          </ol>
          <AdvisoriesSourceNote note={advisories.sourceNote} since={advisories.watchingSince} />
        </div>
      )}
    </section>
  );
...
function AdvisoriesSourceNote({ note, since }: { note: string; since?: string }) {
  return (
    <p
      className="text-small"
      style={{ margin: "var(--space-4) 0 0", color: "var(--color-text-tertiary)" }}
    >
      {note} Hearth checks every 30 minutes
      {since ? ` and has recorded advisories since ${formatAdvisoryDate(since)}` : ""}.
    </p>
  );
}

const PUBLIC_SCOPE_LABEL: Record<PublicAdvisoryItem["scope"], { label: string; help: string }> = {
  system_wide: { label: "District-wide", help: "Affects customers across the water system." },
  localized: { label: "Localized", help: "A listed set of streets or addresses, not the whole system." },
  unknown: { label: "Scope unknown", help: "Hearth couldn't tell how wide this is from the notice." },
};

function PublicAdvisoryRow({ item }: { item: PublicAdvisoryItem }) {
  const scope = PUBLIC_SCOPE_LABEL[item.scope];
  const dates =
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        <span className="chip" title={scope.help}>
          {scope.label}
        </span>
      </div>
      <div style={{ fontWeight: 500, marginTop: 6, color: "var(--color-text-primary)" }}>
        {item.title}
      </div>
      {item.open && item.summary ? (
        <p className="text-small" style={{ margin: "4px 0 0", color: "var(--color-text-secondary)" }}>
          {item.summary}
        </p>
      ) : null}
      {item.open ? (
        <p className="text-small" style={{ margin: "6px 0 0", color: "var(--color-text-primary)" }}>
          Until it&rsquo;s lifted, boil tap water for two minutes before drinking,
          cooking, making ice, or brushing teeth — or use bottled water. Showering,
          laundry, and washing are fine.
        </p>
      ) : null}
      <div
