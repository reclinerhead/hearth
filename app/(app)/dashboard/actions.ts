...
/**
 * Re-run the habitat checks for a house the signed-in user owns.
 *
 * Originally this also fired a Zillow-backed briefing workflow alongside
 * habitat (hence the name), but issue #210 removed the Zillow lookup —
 * the dashboard "Refresh" affordance now re-runs the habitat orchestrator
 * only. Kept the function name and shape so the existing call sites stay
 * stable; behaviourally this is now equivalent to `triggerHabitatRecheck`
 * with the dashboard-specific user-facing copy.
 */
export async function refreshBriefing(
  ...
  try {
    ...
    console.error("refresh: habitat workflow start failed", workflowError);
    return {
      ok: false,
